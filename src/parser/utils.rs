...
pub(super) fn read_mandatory_attribute<T: std::str::FromStr>(node: &Node, attr_name: &str) -> Result<T, XtceParseError> {
...
pub(super) fn read_name_description(nd: &mut NameDescription, node: &Node) {
...
pub(super) fn read_mandatory_name<'a>(node: &'a Node) -> Result<&'a str, XtceParseError> {
    node.attribute("name").ok_or_else(|| get_parse_error("Cannot find mandatory attribute name", node))
}

pub(super) fn read_attribute<T: std::str::FromStr>(node: &Node, attr_name: &str) -> Result<Option<T>, XtceParseError> {
    if let Some(strv) = node.attribute(attr_name) {
        match strv.parse::<T>() {
            Ok(n) => Ok(Some(n)),
            Err(_) => Err(get_parse_error(format!("Cannot parse value '{}' for attribute {}", strv, attr_name), node))
        }
    } else {
        Ok(Option::None)
    }
