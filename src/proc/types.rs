...
use crate::{
    mdb::{
        AggregateDataType, DataEncoding, DataType, NameIdx, NamedItem, TypeData, EnumeratedDataType,
    },
    value::{AggregateValue, ContainerPosition, EngValue, RawValue, ValueUnion, EnumeratedValue},
};

use super::{encodings::extract_encoding, MdbProcError, ProcCtx};

pub(crate) fn extract(ptype: &DataType, ctx: &mut ProcCtx) -> Result<RawValue, MdbProcError> {
    if let DataEncoding::None = ptype.encoding {
        match &ptype.type_data {
            ...
            _ => {
                return Err(MdbProcError::InvalidMdb(format!(
                    "base data type without encoding: {}",
                    ctx.mdb.name2str(ptype.name())
                )));
            }
        }
    } else {
...
fn extract_aggregate(
    ...
    let bit_offset0 = ctx.buf.get_position();
    let start_offset = ctx.start_offset;

    for m in &atype.members {
        let dtype = ctx.mdb.get_data_type(m.dtype);
        let member_rv = extract(dtype, ctx)?;
        println!("Inserting for member {} value {:?}", ctx.mdb.name2str(m.name()), member_rv);
        aggrm.insert(m.name(), member_rv);
    }
    let aggrv = AggregateValue(aggrm);

    let bit_offset1 = ctx.buf.get_position();
    let rv = RawValue {
...
fn from_signed_integer(v: i64, dt: &DataType, _ctx: &ProcCtx) -> Result<EngValue, MdbProcError> {
    ...
    let x = match &dt.type_data {
        ...
        _ => return Err(MdbProcError::InvalidValue(format!("cannot convert integer to {:?}", dt.type_data)))
...
fn from_unsigned_integer(v: u64, dt: &DataType, _ctx: &ProcCtx) -> Result<EngValue, MdbProcError> {
    if let Some(cal) = &dt.calibrator {
        todo!()
    }
    let x = match &dt.type_data {
        ...
        TypeData::AbsoluteTime(_) => todo!(),
        _ => return Err(MdbProcError::InvalidValue(format!("cannot convert unsigned integer to {:?}", dt.type_data)))
    };
...
fn from_aggregate(
    ...
    if let TypeData::Aggregate(atype) = &dt.type_data {
        ...
        return Err(MdbProcError::InvalidValue(serr));
    }
